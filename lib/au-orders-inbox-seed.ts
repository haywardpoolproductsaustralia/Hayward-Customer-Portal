import { createIntake, type IntakeData } from "./au-orders-inbox";

/** Realistic sample extractions: includes a duplicate PO and an unresolved customer. */
export function sampleIntake(): IntakeData[] {
  const now = Date.now();
  ...
  return [
    ...
    },
    {
      ...
      notes: "Customer quoted $389 ea on cells — confirm against contract.",
    ...
    },
    {
      ...
      lines: [
        { raw: "one pool pump, the quiet one you sell", sku: null, description: null, qty: 1, unit: "ea", claimedPrice: null, confidence: "low" },
      ],
...
export async function seedSampleIntake(): Promise<number> {
  const samples = sampleIntake();
  for (const s of samples) await createIntake(s);
