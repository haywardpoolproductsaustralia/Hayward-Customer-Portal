/* lib/recon/queries.ts
   ...
   database (same rule as every other page). Producers:
     ...
     recon:as400_orders     As400Row[]    <- manual upload /api/reconciliation/upload-as400
                                          (swap to portal-sync once the Snowflake
...
interface ArrowMeta { generatedAt?: string; rows?: number; pos?: number }
interface As400Meta { uploadedAt?: string; file?: string; rows?: number; pos?: number }

export async function getReconInputs(): Promise<ReconInputs> {
  const [arrow, as400, shipLines, shipMeta, arrowMeta, as400Meta] = await Promise.all([
    ...
    getJSON<ArrowMeta>("recon:arrow_meta"),
    getJSON<As400Meta>("recon:as400_meta"),
  ]);
  ...
  return {
    ...
    arrowGeneratedAt: arrowMeta?.generatedAt ?? null,
    as400UploadedAt: as400Meta?.uploadedAt ?? null,
  };
}
